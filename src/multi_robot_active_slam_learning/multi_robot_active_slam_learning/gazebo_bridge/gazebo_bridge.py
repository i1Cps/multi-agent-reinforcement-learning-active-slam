...
from multi_robot_active_slam_interfaces.srv import ResetGazeboEnv
import math
...
class GazeboBridge(Node):
    ...
    def initialise_clients(self) -> None:
        ...
        self.slam_reset_client = self.create_client(Reset, "/robot1/slam_toolbox/reset")
    ...
    # Generate new random robot poses.
    def _generate_new_robot_poses(self):
        ...
        selected_indices = np.random.choice(
            len(positions), self.NUMBER_OF_ROBOTS, replace=False
        )
        poses = []

        for robot_pose_index in selected_indices:
            # Select orientations for chosen positions
            orientation_index = np.random.choice(len(orientations[robot_pose_index]))

            sampled_pose = Pose(
                position=positions[robot_pose_index],
                orientation=orientations[robot_pose_index][orientation_index],
            )
            poses.append(sampled_pose)

        return poses

    # Reset the robot's position to a new random pose.
    def _reset_robot_poses(self) -> List[Pose]:
        poses = self._generate_new_robot_poses()
        for robot in range(self.NUMBER_OF_ROBOTS):
            req = SetEntityState.Request()
            # sorry
            req.state.name = f"{self.ROBOT_NAME}{robot+1}"
            req.state.pose = poses[robot]
