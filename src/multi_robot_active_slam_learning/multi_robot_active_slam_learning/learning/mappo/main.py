...
def run():
    ...
    memory = PPOMemory(
        batch_size, N, n_agents, env.agents, n_procs, critic_dims, actor_dims, n_actions
    )
    ...
    while total_steps < MAX_STEPS:
        ...
        while not any(terminal):
            a_p = [
                mappo_agents.choose_action(observation[idx]) for idx in range(n_procs)
            ]
            ...
            action_dict = {
                agent: [list(a[agent]) for a in action] for agent in env.agents
            }
            obs_dict = {
                agent: [list(o[agent]) for o in observation] for agent in env.agents
            }
            new_obs_dict = {
                agent: [list(o[agent]) for o in observation_] for agent in env.agents
            }
            probs_dict = {agent: [list(p[agent]) for p in prob] for agent in env.agents}
