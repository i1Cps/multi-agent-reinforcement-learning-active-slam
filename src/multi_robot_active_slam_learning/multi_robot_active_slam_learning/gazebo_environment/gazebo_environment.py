...
class GazeboEnvironment(Node):
    def __init__(self):
        ...
        self.slam_reset_clients = []
        for i in range(1, self.num_robots + 1):
            self.slam_reset_clients.append(
                self.create_client(Reset, f"/robot{i}/slam_toolbox/reset")
            )
        ...
        # Define quaternions for up, down, left, right orientation, assuming RIGHT as the starting orientation
        ...
        for i in range(self.num_robots):
            self.start_positions.append(Pose(position=self.ROBOT_POSITIONS[i]))
    ...
    def _generate_new_robot_poses(self, num_poses=3):
        sampled_poses = []
        ...
        position_indices = np.random.choice(
            len(positions), size=num_poses, replace=False
        )
        ...
        return sampled_poses  # Return only the first sampled pose

    def _reset_robot_pose(self):
        poses = self._generate_new_robot_poses(num_poses=self.num_robots)
        for i in range(self.num_robots):
            req = SetEntityState.Request()
            req.state.name = f"robot{i+1}"
            req.state.pose = poses[i]
    ...
    def _reset_slam_map(self):
        ...
        for i in range(self.num_robots):
            ...
            self.slam_reset_clients[i].call_async(Reset.Request())
